...
public class birdmodeBlockGenerator : MonoBehaviour
{
...
private void Awake() {
    for (int i = 0; i < blocksToGenerate; i++) {
        ...
        smallBlock.GetComponent<BlockBirdMode>().SetBlockNumberAndSpawn(blockNumber, transform, true);
        ...
        largeBlock.GetComponent<BlockBirdMode>().SetBlockNumberAndSpawn(blockNumber, transform, true);
        blockNumber++;
